...
import { AbiFunction, AbiParam,DecodedOutput } from '@eversdk/core';
...
class OutputDecoder {
  ...
  constructor(output: DecodedOutput, functionAttributes: AbiFunction) {
    this.output = output;
  ...
  decode() {
    const outputDecoded = this.decodeTuple(
      ...
      // console.log(this.output, this.functionAttributes,outputDecoded)
  ...
  static autoDecode(decoded_msg: { body_type: string; name: any; value: DecodedOutput; }, contract_abi: { functions: any[]; events: any[]; }) {
